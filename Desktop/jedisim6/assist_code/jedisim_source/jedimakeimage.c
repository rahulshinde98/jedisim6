...
typedef struct {
  ...
  bool		looped;		//the current state of the indecators
  int		size;		//size of the queue
  int		*queue;		//the actual queue
  pthread_mutex_t *lock;	//lock to protect the array
} queue;
...
void queue_put(queue *q, int arg);
void queue_destroy(queue *q);
void queue_print(queue *q); //debugging
...
int		NUMBANDS;		//The number of bands of the final image
...
int main(int argc, char *argv[]) {
	...
	//Transform Distort and Paste
	transformDistortPaste(argc, argv);
...
//Transform, Distort, and Paste all galaxies into one array
void transformDistortPaste(int argc, char *argv[]){
  int     	status = 0;	//CFITSIO status
  long int	naxis = 2;	//CFITSIO variables
  fitsfile 	*ffptr;		//pointer to final image as fits
  FILE		*galfile;	//pointer to the catalog file 
  char		buffer1[1024];	//buffer for reading in strings
  ...
  //parse gallist file
  if((galfile = fopen(gallist_path, "r")) == NULL){
  ...
  ngalaxies = 0;
  galaxy temp;
  while(fscanf(galfile, "%s\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f", &buffer1, &temp.x, &temp.y, &temp.angle, &temp.redshift, &temp.pixscale, &temp.old_mag, &temp.old_r50, &temp.new_mag, &temp.new_r50) > 0)
    ngalaxies++;
  ...
  galaxies = (galaxy *) calloc(nthreads, sizeof(galaxy));
  if(galaxies == NULL){
    fprintf(stderr, "Error: could not allocate memory for galaxy list.");
    exit(1);
  }

  pthread_t tid[nthreads]; //thread id
  ...
  sem_init(&mysem, 0, nthreads); //initialize semaphore
  queue_init(&myq, nthreads); //initialize queue

  int g;
  int glst[nthreads];
  for (g = 0; g < nthreads; g++) {
    galaxies[g].image = (char *) calloc(maxstamp, sizeof(char)); //allocate memory for file paths
    if (galaxies[g].image == NULL) {
      fprintf(stderr, "Error: could not allocate memory for galaxy image path.");
      exit(1);
    }
    queue_put(&myq, g); //fill the queue
    glst[g] = g; //to pass the index into each thread (since the for loop index changes)
  }
  //Loading bar variables
  int load;
  int chunk = ngalaxies /26;
  int percent;
  fprintf(stdout, "\n");
  ...
  //Then distorting and finally pasting each into the final image
  //By reading in each galaxy here, we keep the retained memory low
  for(g = 0; g < ngalaxies; g++){
    ...
    // Fill galaxy with information
    fscanf(galfile, "%s\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f", &buffer1, &galaxies[ind].x, &galaxies[ind].y, &galaxies[ind].angle, &galaxies[ind].redshift, &galaxies[ind].pixscale, &galaxies[ind].old_mag, &galaxies[ind].old_r50, &galaxies[ind].new_mag, &galaxies[ind].new_r50);
    strcpy(galaxies[ind].image, buffer1);

    //create a thread
    pthread_create(&tid[ind], &attr, runThread, (void *)&glst[ind]);

    //update user
    if (g % 10000 == 0) {
      fprintf(stdout, "Transformed, Distorted, and Pasted image %i/%i: ", g, ngalaxies);
      fprintf(stdout, "[");
      percent = (int) g / chunk;
      for (load = 0; load < percent; load++) {
        fprintf(stdout, "=");
      }
      for (load = percent; load < 25; load++) {
        fprintf(stdout, " ");
      }
      fprintf(stdout, "]\n");
      fflush(stdout);
  ...
  int check;
  sem_getvalue(&mysem, &check);
  while (check != nthreads) {
    sem_getvalue(&mysem, &check);
  }

  fclose(galfile); //Close catalog file
...
void queue_init(queue *q, int size) {
	...
	if ((*q).lock == NULL) {
		fprintf(stderr, "Error: Unable to allocate memory for Queue lock.");
		exit(1);
	}
...
int queue_get(queue *q) {
	...
	//check for empty
	if ((*q).front -1 == (*q).rear && !((*q).looped)) {
		fprintf(stderr, "Error: Attempt to get from empty queue.");
		exit(1);
	}
	pthread_mutex_unlock((*q).lock);//unlock
	//return front element
	if ((*q).front == 0) {return (*q).queue[(*q).size -1];} //if the queue just looped, return the last element
	else {return (*q).queue[(*q).front -1];} //otherwise return the element before the new front
}

//Put a New Element in Queue
void queue_put(queue *q, int arg) {
	pthread_mutex_lock((*q).lock);//lock
	//set rear element
	(*q).queue[(*q).rear] = arg;
...
void queue_print(queue *q) {
	fprintf(stderr, "f: %d, r: %d\n",(*q).front, (*q).rear);
	int i;
	for(i=0;i<(*q).size;i++){
		fprintf(stderr,"%d, ", (*q).queue[i]);
	}
	fprintf(stderr," \n");
}
...
float *threadDistort(int gal, float *galimage) {
    ...
    for(gr = ngr; gr > 0; gr--){
      ...
      long int     gs_wd = (ngr == gr ? nx : gexp << (b[0]*gr));
      long int     gs_ht = (ngr == gr ? ny : gexp << (b[0]*gr));
      long int     s_gs_s= gexp << (b[0]*(gr-1));        //sub-gridsquare sieve length
      int subrows_per_row = (ngr == gr ? (nx >> b[gr-1]) : gexp);
      int subcols_per_col = (ngr == gr ? (nx >> b[gr-1]) : gexp);
...
void *threadPaste(int g, float *galimage) {
  ...
  int i;
  for (i = locknum1; i <= locknum2; i++) {
    pthread_mutex_lock(&bandlocks[i]);
  }
...
void setupTransform(int argc, char *argv[]) {
  //declare variables
  FILE	*config_fp;      //config file
  char	buffer[1024], buffer3[1024]; //buffers for reading in strings
    ...
    if(buffer[0] != '#'){
      ...
      if(strcmp(buffer3,"num_source_images")==0){
        ...
        if(paths == NULL){
          ...
          if(strlen(temp) > maxstamp) {
            maxstamp = strlen(temp);//get the largest stamp path size
          }
  ...
  maxstamp += 1; //add a null terminator to the stamp paths
  fclose(config_fp); 
...
void setupDistort(int argc, char *argv[]) {
  ...
  //initialize memory for all the lenses
...
//fprintf(stdout,"alpha: %f\n", alpha);
  ...
  for(gr = 0; gr < ngr; gr++){
	...
	//loop over subgrid
	rect bounding_box;
	for(srow = 0; srow < gexp; srow++){
	  for(scol = 0; scol < gexp; scol++){
	    if(gr==0){
	      //get deflection angle
	      float alphax = 0, alphay = 0;
	      get_alpha((row*gexp+srow) << BNSUBPX, (col*gexp+scol) << BNSUBPX, nlenses, lenses, &alphax, &alphay);
		...
		if(alphax < bounding_box.xmin)
	      ...
	      int index = (ngy[gr-1]*(col*gexp+scol))+row*gexp+srow;                            
...
void setupPaste() {
	...
	NUMBANDS = nthreads * (nthreads -1) /4;
	fprintf(stdout, "Number of bands of final image: %d\n", NUMBANDS);
